...
dotnet build
# To run on Android
dotnet build -f net11.0-android -t:Run
# To run on iOS
dotnet build -f net11.0-ios -t:Run
```
...
Thus you can easily write unit tests in a vanilla `net11.0` Xunit
project, such as:
...
The above views in a `net11.0` project are not real UI, while
`net11.0-android` and `net11.0-ios` projects get the full
...
First, you can write the cross-platform part for a vanilla `net11.0`
class library:
